...
$stmt = $conn->prepare("
    SELECT v.Data, v.Partenza, v.Note, u.Nome, u.Cognome
	FROM Prenotazione p
	JOIN Viaggio v ON v.ID = p.ID_Viaggio
	JOIN Utente u ON u.ID = v.ID_Autista
	WHERE p.ID_Passeggero = ? AND p.Stato = 'Accettata' AND v.Completato = 0
...
<div class="main">
  <div class="container">
    <h2>I tuoi viaggi confermati</h2>
 
    <?php if (count($viaggi) === 0): ?>
      <p class="no-data">Non hai viaggi confermati al momento.</p>
    <?php else: ?>
      <?php foreach ($viaggi as $v): ?>
        <div class="viaggio-box">
          <strong>Autista:</strong> <?= htmlspecialchars($v['Nome'] . ' ' . $v['Cognome']) ?><br>
          ...
          <form method="POST" action="annulla_prenotazione.php" onsubmit="return confirm('Sei sicuro di voler annullare la prenotazione?');">
            <input type="hidden" name="id_viaggio" value="Note: <?= isset($v['Note']) && $v['Note'] !== '' ? htmlspecialchars($v['Note']) : 'Nessuna' ?>">
            <button type="submit">❌ Annulla prenotazione</button>
          </form>
        </div>
      <?php endforeach; ?>
    <?php endif; ?>
